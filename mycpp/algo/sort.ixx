...
{
    // 合并函数，将有序数组src1和src2合并到长度为（src1_len +
    // src2_len2）result数组中
    static auto merge = [&compare](Type *src1, size_t src1_len, Type *src2,
                                   size_t src2_len,
    ...
    for (size_t mid = 1; mid < length; mid *= 2)
    {
        ...
        {
            merge(buffer1 + i, mid, buffer1 + i + mid,
                  // 末尾的组可能个数不够中值
                  (lim - i) >= (2 * mid) ? mid : (lim - i - mid), buffer2 + i);
        }
...
void QuickSort(Type *src, size_t length, Compare compare) noexcept
{
    ...
    static auto partition = [&compare](Type *nums, size_t start,
                                       size_t end) noexcept -> size_t {
        // 设定基准
        size_t pivot = start;
    ...
    // 可以递归的函数
    static void (*quick)(Type *, size_t, size_t) =
        [](Type *nums, size_t start, size_t end) noexcept -> void {
        // 递归的条件
        if (end > start)
        {
