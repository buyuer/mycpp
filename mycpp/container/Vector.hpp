...
public:
    ...
    void* memset(void* src, int c, size_t n) { return ::memset(src, c, n); }
...
public:
    ...
    explicit Vector() { init_(VECTOR_DEFAULT_INCREMENT_COUNT, 0); }
    ...
    Vector(const Vector& other) { copy_(other); }
    ...
    Vector& operator=(const Vector& other) { return copy_(other); };
    ...
    const T& at(size_type pos) const { return operator[](pos); }
    ...
    iterator erase(const iterator pos) { return erase(pos, pos + 1); }
    ...
    const iterator begin() const noexcept { return iterator(data_); }
...
private:
    inline static size_t SIZE_BYTE(size_t size) { return size * sizeof(T); }
